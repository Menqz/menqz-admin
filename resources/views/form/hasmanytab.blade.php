<div id="has-many-{{$column}}" class="nav-tabs-custom has-many-{{$column}}">
    ...
    <template class="{{$column}}-tab-tpl">
        <li class="new nav-item" id="tab_{{ $relationName . '_new_' . \MenqzAdmin\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
            <a class="nav-link" href="#{{ $relationName . '_new_' . \MenqzAdmin\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}" data-bs-toggle="tab">
                &nbsp;New {{ \MenqzAdmin\Admin\Form\NestedForm::DEFAULT_KEY_NAME }} <i class="icon-exclamation-circle text-red hide"></i>
            </a>
        </li>
    </template>
    <template  class="{{$column}}-tpl">
        <div class="tab-pane fields-group new" id="{{ $relationName . '_new_' . \MenqzAdmin\Admin\Form\NestedForm::DEFAULT_KEY_NAME }}">
            {!! $template !!}
            @if($options['allowDelete'])
            <div class="form-group form-delete-group">
                <label class="{{$viewClass['label']}} form-label"></label>
